...
from utils.code import ADD_OPTIMIZERS, INITIALIZER, MODELS_SET_MODE, MODELS_SET_ZERO_GRAD, OPTIMIZER_STEP, log, SAVE_LOG
from utils.code import LOAD_WEIGHT, SAVE_CHECKPOINT, LOAD_CHECKPOINT
from datasets.dataset import Dataset
from utils.loss import binary_cross_entropy_loss, dice_loss, dm_loss, contect_loss, kl_divergence_loss, l2_loss, orient_loss, penetrate_loss, refine_loss
from models.components.mano.utils import get_mano_result, mano_layer
from utils.utils import clone_detach_dict_tensor, estimated_distance_maps, get_deformed_obj_point_cloud, get_hand_motion_mask, get_padding_mask, params_to_device, relative_3d_orientation_diff
...
def train_one_epoch(epoch):
    ...
    for batch_idx in tqdm.tqdm(range(loader_len)):
        ...
        frame_mask, frame_mask_seq, frame_mask_seq_cond = get_padding_mask(
            B, CFGS.max_frame, true_frame_len, pred_frame_len, DEVICE)
        ...
        pred_motion_lhand, pred_motion_rhand, pred_motion_obj = MODELS["motion_generator"](
            ...
            CFGS.max_frame,
            False,
            hand_motion_mask, frame_mask_seq_cond,
            data["lh_motion"], 
            data["rh_motion"], 
            data["obj_motion"])
        ...
        true_mano_lhand = get_mano_result(
            data["lh_motion"], lhand_mano_layer, hand_motion_mask["mask_lhand"], frame_mask,
            CFGS.max_frame, B, DEVICE)
    
        true_mano_rhand = get_mano_result(
            data["rh_motion"], rhand_mano_layer, hand_motion_mask["mask_rhand"], frame_mask,
            CFGS.max_frame, B, DEVICE)
        
        pred_mano_lhand = get_mano_result(
            pred_motion_lhand, lhand_mano_layer, hand_motion_mask["mask_lhand"], frame_mask,
            CFGS.max_frame, B, DEVICE)

        pred_mano_rhand = get_mano_result(
            pred_motion_rhand, rhand_mano_layer, hand_motion_mask["mask_rhand"], frame_mask,
            CFGS.max_frame, B, DEVICE)
        ...
        lhand_dist_map_diff = estimated_distance_maps(
            ...
            hand_motion_mask["mask_lhand"], frame_mask,
            CFGS.tau)
        ...
        rhand_dist_map_diff = estimated_distance_maps(
            ...
            hand_motion_mask["mask_rhand"], frame_mask,
            CFGS.tau)
        ...
        ref_motion_lhand, ref_motion_rhand = MODELS["hand_refinement_network"](
            ...
            ref_point_cloud_pred,
            hand_motion_mask, frame_mask_seq
        )
...
def train(start_epoch=0):
    global DEVICE, MODELS, OPTIMIZERS
